...
const std::unordered_map<Plazza::PizzaType, Plazza::reference> pizzaRef = {
...
Plazza::Pizza::Pizza(std::string const s, double const factor)
{
    int pos = s.find(':');
    ...
    _time = std::chrono::milliseconds((int)(pizzaRef.at(_type)._time * factor * 1000));
    _pack = s;
    _unpack = pizzaRef.at(_type)._name + " " + sizeRef.at(_size);
}

Plazza::Pizza::Pizza(PizzaType const type, PizzaSize const size, double const factor) noexcept : _type{type}, _size{size}
{
    _type = type;
    _size = size;
    _time = std::chrono::milliseconds((int)(pizzaRef.at(_type)._time * factor * 1000));
...
std::string Plazza::Pizza::pack(void) const noexcept
{
    return (_pack);
}

std::string Plazza::Pizza::unpack(void) const noexcept
{
    return (_unpack);
}
