...
namespace Plazza {
    struct reference
    {
        reference(std::string const &name, std::vector<Ingredient> const &ingredient, int time)
        : _name(name),
    ...
    class Pizza
    {
        public:
            ...
            std::string pack(void) const noexcept;
            std::string unpack(void) const noexcept;
